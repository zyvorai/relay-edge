...
Seed plant inventory, enable Publish into Relay, run scenarios (lowtank, fire, comms, vision, gas).

## When to use it

- Open **Firewater** when the job matches this screen
- Prefer the product home / Get started panel if you are unsure where to begin
- Confirm health and auth tokens if probes fail

## How to get there

- UI path: `/ui/` → **Firewater** (or matching nav tab)
...
2. Navigate to **Firewater**.
3. Complete the on-screen fields / actions for this surface (Seed plant inventory, enable Publish into Relay, run scenarios (lowtank, fire, comms, vision, gas).).
