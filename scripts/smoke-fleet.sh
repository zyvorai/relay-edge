...
curl -fsSk "$EDGE/healthz" | python3 -c 'import json,sys; d=json.load(sys.stdin); assert "fleet" in d.get("modules",[]); print("health modules", ",".join(d["modules"]), "version", d.get("version",""))'
curl -fsSk -o /dev/null -w "fleet ui %{http_code}\n" "$EDGE/ui/fleet.html"

curl -fsSk "$EDGE/v1/fleet/catalog" | python3 -c '
import json,sys
...
curl -fsSk -X POST "$EDGE/v1/fleet/scenario" -H 'content-type: application/json' -d '{"scenario":"blackout"}' | python3 -c '
import json,sys
d=json.load(sys.stdin)
...
curl -fsSk -X POST "$EDGE/v1/fleet/scenario" -H 'content-type: application/json' -d '{"scenario":"amr_lost"}' | python3 -c '
import json,sys
d=json.load(sys.stdin)
types=[e.get("type") for e in (d.get("events") or [])]
